...
# Fungsi untuk menghasilkan nomor tiket berdasarkan nomor loket
def generate_ticket(loket_number):
    # Mendapatkan nilai terkini dari file
    usage_count = read_usage_count()

    # Di sini Anda dapat menulis logika untuk menghasilkan nomor tiket
    # Misalnya, menambahkan nomor berikutnya dari loket tertentu
    # Setelah itu, Anda dapat memperbarui nilai variabel display yang sesuai

    loket_letters = ['A', 'B', 'C', 'D', 'E']
    ticket_number = f"{loket_letters[loket_number - 1]}{usage_count+1}"

    if loket_number == 1:
        loket_1_display_var.set(f"{ticket_number} Loket 1")
    elif loket_number == 2:
        loket_2_display_var.set(f"{ticket_number} Loket 2")
    elif loket_number == 3:
        loket_3_display_var.set(f"{ticket_number} Loket 3")
    elif loket_number == 4:
        loket_4_display_var.set(f"{ticket_number} Loket 4")
    elif loket_number == 5:
        loket_5_display_var.set(f"{ticket_number} Loket 5")

# Fungsi untuk mencetak nomor tiket ke terminal
def print_ticket(loket_number):
    print(f"Nomor Tiket Loket {loket_number}: {get_display_var(loket_number).get()}")

# Fungsi bantu untuk mendapatkan nilai variabel display yang sesuai
def get_display_var(loket_number):
    if loket_number == 1:
        return loket_1_display_var
    elif loket_number == 2:
        return loket_2_display_var
    elif loket_number == 3:
        return loket_3_display_var
    elif loket_number == 4:
        return loket_4_display_var
    elif loket_number == 5:
        return loket_5_display_var
